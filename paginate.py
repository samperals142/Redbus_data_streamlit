...
def main():
    ...
    # Limit the number of rows displayed at once to avoid performance issues
    rows_to_display = st.slider("Number of rows to display", min_value=10, max_value=100, value=20) # paginate for 20 items Default
    
    # Convert 'Route_Link' column into clickable links using Markdown
    df['Route_Link'] = df['Route_Link'].apply(lambda x: f"[Link]({x})")

    st.subheader(f"Displaying Top {rows_to_display} Bus Routes")    # paginate
    #st.markdown(df.head(rows_to_display).to_markdown(index=False), unsafe_allow_html=True)  # paginate
    ...
    if selected_column != '' and selected_value != '':  # If both column and value are selected
        ...
        #st.data_editor(filtered_df,use_container_width=True, height=700) #using dataeditor
        
        st.markdown(filtered_df.head(rows_to_display).to_markdown(index=False), unsafe_allow_html=True) # using markdown with route link clickable
        ...
        #st.data_editor(df,use_container_width=True, height=700) #using Dataeditor
        
        st.markdown(df.head(rows_to_display).to_markdown(index=False), unsafe_allow_html=True) # using markdown with route link clickable
